...
for transition in transitions:
    dfa_list.append([transition[0],transition[1]])

# verificam daca e dfa

for i in range(len(dfa_list)):
    for j in range(i+1,len(dfa_list)):
        if dfa_list[i][0] == dfa_list[j][0] and dfa_list[i][1] == dfa_list[j][1]:
            print("ITS A NFA NOT A DFA")
            exit(0)
